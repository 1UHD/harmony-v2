...
class PlaylistCog(commands.Cog):
    ...
    async def playlist_create(self, ctx: commands.Context, name: str) -> None:
        if not playlist_manager.is_available(name=name):
            await embeds.send_embed(title="This playlist name is already in use.", context=ctx)
            return

        playlist_manager.create_playlist(name=name)

        await embeds.send_embed(title=f"Playlist {name} has been created.", context=ctx)
    ...
    async def playlist_addcurrent(self, ctx: commands.Context, playlist: str) -> None:
        ...
        if playlist_manager.is_available(playlist):
            await embeds.send_error(title=f"Playlist {playlist} doesn't exist.", context=ctx)
            return
        
        current = queue.get_current()

        if isinstance(current, MP3Song):
            await embeds.send_error(title=f"MP3 songs cannot be stored in Playlists as of right now.", context=ctx)
            return
        ...
        await embeds.send_embed(title=f"{current.title} has been added to {playlist_object.name}.", context=ctx)
    ...
    async def playlist_add(self, ctx: commands.Context, playlist: str, prompt: str) -> None:
        ...
        if is_link == "yt_link":
            message = await embeds.send_embed("Adding song to playlist.", context=ctx, color=discord.Color.yellow())
            yt_url = prompt
    ...
    async def playlist_queue(self, ctx: commands.Context, playlist: str) -> None:
        ...
        if playlist_object.get_song_amount() < 1:
            await embeds.send_error(title=f"{playlist_object.name} is empty.", context=ctx)
            return
        
        await ctx.interaction.response.defer()
        logger.debug("interaction was saved")
        
        message = await embeds.send_embed(title=f"Adding {playlist_object.get_song_amount()} song{'s' if playlist_object.get_song_amount() != 1 else ''}", context=ctx, color=discord.Color.yellow())
    ...
    async def playlist_remove(self, ctx: commands.Context, playlist: str, index: int) -> None:
        ...
        playlist_object = playlist_manager.get_playlist_object_by_name(name=playlist)
        if playlist_object.get_song_amount() < 1:
            await embeds.send_error(title=f"{playlist} is empty. You can't remove a song from an empty playlist.", context=ctx)
            return
        
        if playlist_object.get_song_amount() < index:
            await embeds.send_error(title=f"Song {index} does not exist.", context=ctx)
            return

        song_title = playlist_object.songs[index-1].title
        playlist_manager.remove_song(pl=playlist_object, index=index)

        await embeds.send_embed(title=f"{song_title} has been removed.", context=ctx)
    ...
    async def playlist_delete(self, ctx: commands.Context, playlist: str) -> None:
        ...
        await embeds.send_embed(title=f"Playlist {playlist} has been deleted.", context=ctx)
    ...
    async def playlist_info(self, ctx: commands.Context, playlist: str) -> None:
        ...
        if playlist_object.get_song_amount() < 1:
            await embeds.send_error(title="Playlist is empty.", context=ctx)
            return
    ...
    async def playlist_list(self, ctx: commands.Context) -> None:
        ...
        if len(playlists) == 0:
            await embeds.send_error(title="No playlists available.", context=ctx)
            return

        await embeds.send_embed(title="Available playlists:", description="".join(f"{p.name} - {len(p.songs)} songs\n" for p in playlists), context=ctx)
