...
class MP3Cog(commands.Cog):
    ...
    async def mp3(self, ctx: commands.Context) -> None:
        if not ctx.invoked_subcommand:
            await embeds.send_error(title="Wrong usage, see `/help mp3` for more info.", context=ctx)
    ...
    async def mp3_file(self, ctx: commands.Context, filepath: str) -> None:
        if not settings.mp3_path:
            await embeds.send_error(title="Missing path permission!",
        ...
        if status == "error_not_mp3":
            await embeds.send_error(title="Path does not contain an MP3 file", context=ctx)
            return
        
        elif status == "error_not_found":
            await embeds.send_error(title="File does not exist", context=ctx)
            return
        
        elif status == "error_adding":
            await embeds.send_error(title="File couldn't be added", context=ctx)
            return
        
        else:
            await embeds.send_embed(title=f"{status} has been added to the queue", context=ctx)
    ...
    async def mp3_directory(self, ctx: commands.Context, directory: str) -> None:
        ...
        if "mp3" in directory:
            await embeds.send_error(title="Provided file path, expected directory", context=ctx)
            return
        ...
        if status == "error_not_found":
            await embeds.send_error(title="Directory does not exist", context=ctx)
            return
        
        else:
            await embeds.send_embed(title="Added all available songs", context=ctx)
